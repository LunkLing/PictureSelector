...
public class PicturePreviewActivity extends PictureBaseActivity implements
    ...
    private TextView tv_img_num, tv_title, tv_no, tv_ok;
    ...
    public void onSelectNumChange(boolean isRefresh) {
        ...
        tv_ok.setSelected(enable);
        tv_no.setVisibility(!enable ? View.VISIBLE : View.GONE);
        tv_ok.setVisibility(enable ? View.VISIBLE : View.GONE);
        if (enable) {
            tv_ok.setBackgroundResource(R.drawable.picture_btn_ok_preview);
            if (numComplete) {
